...
for np in "${NP_VALUES[@]}"; do
  for level in "${MAX_LEVELS[@]}"; do
    ...
    mpirun -np ${np} $WORKDIR/samurai/build_mpi/demos/FiniteVolume/finite-volume-advection-2d --Tf 0.05 --min-level 5 --max-level ${level} --timers > output_slurm_np${np}_level${level}.txt 2>&1
